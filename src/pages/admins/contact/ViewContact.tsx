...
export default function ViewContact() {
    ...
    const statusOrder: ('new' | 'processing' | 'resolved')[] = ['new', 'processing', 'resolved'];
    ...
    const handleStatusChange = (status: 'new' | 'processing' | 'resolved') => {
        // Get the index of current and new status in the statusOrder array
        const currentStatusIndex = statusOrder.indexOf(contact.status);
        const newStatusIndex = statusOrder.indexOf(status);

        // Validate that the new status is not a previous state
        if (newStatusIndex <= currentStatusIndex) {
            toast.error('Không thể chuyển về trạng thái trước đó!');
            return;
        }
